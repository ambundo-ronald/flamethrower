...
Sales Assistant is a Frappe / ERPNext app for customer sales history, item pricing, smart recommendations, and draft Quotation or Sales Order creation.

The repository root is now arranged as an installable Frappe app for Frappe Cloud and bench. The earlier Express API and Vue prototype are preserved under `legacy/` for reference.
...
```bash
bench get-app https://github.com/ambundo-ronald/sales_assistant_rest_api_quotes.git
bench --site your-site.local install-app sales_assistant
...
```bash
bench get-app /path/to/sales_assistant_rest_api_quotes
bench --site your-site.local install-app sales_assistant
```
...
```text
sales_assistant/        Frappe app package
